...
exports.createCourse = async (req, res) => {
    try {
        ...
        console.log("Extracted fields:", { 
            ...
            price,
            status,
            instructionsProvided: !!_instructions,
            tagProvided: !!_tag
        });
        ...
        if (!courseName || !courseDescription || !category) {
            ...
            return res.status(400).json({
                success: false,
                message: 'Essential fields are required: courseName, courseDescription, category',
                missingFields
            });
        }
        ...
        try {
            ...
            // Define a default thumbnail URL in case uploading fails
            const DEFAULT_THUMBNAIL = "/assets/images/default-course-thumbnail.svg";
            ...
            if (thumbnail) {
                try {
                    ...
                    if (!uploadSuccess) {
                        console.log("All upload attempts failed. Using default thumbnail.");
                    }
                } catch (error) {
                    console.error("Error in thumbnail upload process:", error);
                    console.log("Using default thumbnail due to upload errors");
                }
            }
            ...
            console.error("Error in category/course creation:", error);
            return res.status(500).json({
                success: false,
                message: 'Error in category or course creation',
                error: error.message
            });
        }
    } catch (error) {
